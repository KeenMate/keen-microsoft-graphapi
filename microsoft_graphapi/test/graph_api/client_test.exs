defmodule GraphApi.ClientTest do
  ...
  describe "new/1 without config (delegated)" do
    test "creates client without config when env is not set" do
      ...
      original = Application.get_env(:keen_microsoft_graphapi, :config)
      Application.delete_env(:keen_microsoft_graphapi, :config)
      ...
      try do
        ...
        assert to_string(client.options[:base_url]) =~ "graph.microsoft.com/v1.0"
      after
        if original, do: Application.put_env(:keen_microsoft_graphapi, :config, original)
      end
    end
