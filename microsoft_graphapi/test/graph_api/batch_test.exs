defmodule GraphApi.BatchTest do
  use ExUnit.Case, async: true

  alias GraphApi.Batch
  alias GraphApi.Users
  alias GraphApi.Groups
  ...
  describe "schema casting" do
    test "casts responses when :as is provided", %{client: client} do
      {:ok, responses} =
        ...
        |> Batch.add("2", Groups.get_query("group-123", as: GraphApi.Schema.Group))
        |> Batch.execute(client: client)

      user_resp = Batch.get(responses, "1")
      assert %{"value" => [%GraphApi.Schema.User{} | _]} = user_resp.body

      group_resp = Batch.get(responses, "2")
      assert %GraphApi.Schema.Group{display_name: "Engineering"} = group_resp.body
    end
  ...
  describe "_query functions" do
    ...
    test "_query functions capture :as and :query options" do
      ...
      req = Users.list_query(query: query, as: GraphApi.Schema.User)

      assert req.as == GraphApi.Schema.User
