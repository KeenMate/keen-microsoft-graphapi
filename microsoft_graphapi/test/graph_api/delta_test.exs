defmodule GraphApi.DeltaTest do
  ...
  describe "schema casting" do
    test "casts items with :as option", %{client: client} do
      {:ok, page} = Delta.query("/users/delta", client: client, as: GraphApi.Schema.User)

      assert [%GraphApi.Schema.User{display_name: "Alice"} | _] = page.items
    end
    ...
    test "does not cast @removed items", %{client: client} do
      ...
      updated = Enum.find(changes.items, fn
        %GraphApi.Schema.User{} -> true
        _ -> false
      end)
      assert %GraphApi.Schema.User{display_name: "Bob Updated"} = updated
