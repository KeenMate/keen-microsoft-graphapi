...
defmodule GraphApi.Schema.Beta.Organization do
  @moduledoc """
  ...
  @type t :: %__MODULE__{
          ...
          verified_domains: [GraphApi.Schema.Beta.VerifiedDomain.t()] | nil,
          assigned_plans: [GraphApi.Schema.Beta.AssignedPlan.t()] | nil
        }
    ...
    %__MODULE__{
      ...
      verified_domains:
        case map["verifiedDomains"] do
          nil -> nil
          items -> Enum.map(items, &GraphApi.Schema.Beta.VerifiedDomain.from_map/1)
        end,
      assigned_plans:
        case map["assignedPlans"] do
          nil -> nil
          items -> Enum.map(items, &GraphApi.Schema.Beta.AssignedPlan.from_map/1)
        end
    }
  end
    ...
    [
      ...
      {"verifiedDomains",
       case struct.verified_domains do
         nil -> nil
         items -> Enum.map(items, &GraphApi.Schema.Beta.VerifiedDomain.to_map/1)
       end},
      {"assignedPlans",
       case struct.assigned_plans do
         nil -> nil
         items -> Enum.map(items, &GraphApi.Schema.Beta.AssignedPlan.to_map/1)
       end}
    ]
