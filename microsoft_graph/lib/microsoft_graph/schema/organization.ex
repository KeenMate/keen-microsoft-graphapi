...
defmodule MicrosoftGraph.Schema.Organization do
  @moduledoc """
  ...
  @type t :: %__MODULE__{
          ...
          verified_domains: [MicrosoftGraph.Schema.VerifiedDomain.t()] | nil,
          assigned_plans: [MicrosoftGraph.Schema.AssignedPlan.t()] | nil
        }
    ...
    %__MODULE__{
      ...
      verified_domains:
        case map["verifiedDomains"] do
          nil -> nil
          items -> Enum.map(items, &MicrosoftGraph.Schema.VerifiedDomain.from_map/1)
        end,
      assigned_plans:
        case map["assignedPlans"] do
          nil -> nil
          items -> Enum.map(items, &MicrosoftGraph.Schema.AssignedPlan.from_map/1)
        end
    }
  end
    ...
    [
      ...
      {"verifiedDomains",
       case struct.verified_domains do
         nil -> nil
         items -> Enum.map(items, &MicrosoftGraph.Schema.VerifiedDomain.to_map/1)
       end},
      {"assignedPlans",
       case struct.assigned_plans do
         nil -> nil
         items -> Enum.map(items, &MicrosoftGraph.Schema.AssignedPlan.to_map/1)
       end}
    ]
