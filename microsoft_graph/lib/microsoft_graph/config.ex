defmodule MicrosoftGraph.Config do
  @moduledoc """
  ...
  * `:base_url` - Graph API base URL (default: derived from `api_version`)
  ...
  ## Examples
      ...
      config = MicrosoftGraph.Config.from_env!()
  ...
  Reads from `Application.get_env(:microsoft_graph, :config)`.
  Raises if the configuration is missing or invalid.
  """
  @spec from_env!() :: t()
  def from_env! do
    opts =
      Application.get_env(:microsoft_graph, :config) ||
        raise "Missing :microsoft_graph :config in application environment"
